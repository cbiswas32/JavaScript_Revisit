...
const playAutomatically = () => {
    let compMoveAuto = computerMoveGenerator();
    moveLogic(compMoveAuto);   
}

let refreshIntervalId;

const autoPlay = () => {
    playBtnEl =  document.querySelector(".autoplay");
    console.log(playBtnEl.innerText);
    if(playBtnEl.innerText == "Auto Play"){
        playBtnEl.innerText = "Stop Play";
        refreshIntervalId = setInterval(playAutomatically,1000);
    }
    else{
        playBtnEl.innerText = "Auto Play";
...
document.body.addEventListener("keyup", (e)=> {
     if(e.key === 'Backspace'){
        document.querySelector(".reset-confirmation-conatiner").style.display = "block";
        document.querySelector(".yes-reset").addEventListener("click", () => {
            resetScore();
            document.querySelector(".reset-confirmation-conatiner").style.display = "none";
        });
